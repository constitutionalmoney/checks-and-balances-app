...
function transition(
  ...
  expectedState: VerusJobState,
  nextState: VerusJobState,
): VerusJobState {
  return applyTransition("verus_job", command, currentState, [expectedState], nextState);
}

export function claimVerusJob(currentState: VerusJobState): VerusJobState {
  return transition("claimVerusJob", currentState, "pending", "claimed");
}
...
export function recordRetryableVerusFailure(currentState: VerusJobState): VerusJobState {
  return transition("recordRetryableVerusFailure", currentState, "readback", "retryable_failed");
}

export function recordTerminalVerusFailure(currentState: VerusJobState): VerusJobState {
  return transition("recordTerminalVerusFailure", currentState, "readback", "terminal_failed");
}

export function markVerusReorgPending(currentState: VerusJobState): VerusJobState {
  return transition("markVerusReorgPending", currentState, "readback", "reorg_pending");
}
